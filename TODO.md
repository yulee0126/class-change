...
### 6.2 課表校對 GUI
- [ ] 匯入 PDF 後，開一個「課表校對」畫面：搜尋／選一位老師 → 顯示他一週課表
      （星期一～五 × 節次 1–10 的表格）
- [ ] 每格可直接編輯 科目／班級／地點／備註（含 `(兼)`/`(輔)` 標記）
- [ ] 存檔 → 寫回課表設定檔（`storage.save_timetable`，即目前的課表 JSON）
...
### 6.4 輸出 Excel 內容調整　✅ 決定（對調備註已完成／Phase A；J 欄與 record 待 Phase B）
- [x] 對調備註格式：`與{對方}老師調課` → `與{對方}老師 {對方科目} 調課`
      （`builder.py` 第 71、75 行；teacher_a 的列插 `leg.subject_b`，teacher_b 的列插 `leg.subject_a`）
- [ ]（Phase B）代課腳：查 `leg.orig_teacher` 在 `leg.slot`（星期＋節次）當下課表的 `Slot.note`
      （沿用 P5 的 `(兼)`/`(輔)` 解析結果），若含 `(兼)` 或 `(輔)`：
      - `(兼)` → 「原老師」「代課老師」教師單那一列 ＋ 班級單那一列，J 欄都加藍字／標楷／粗體「兼課」
      - `(輔)` → 同上三處，J 欄加「八」
      - **決定**：三張單（原老師、代課老師、班級單）只要出現這節課的列都要標，且不影響原本 A–I 內容
      - J 欄先前在 P4 做過「代課老師簡稱」後來移除（見 `test_p4.py`
        `"J 欄簡稱已取消輸出"`），這次重新啟用但用途不同
      - 暫訂：J 欄**不**納入 `print_area`（維持 `A1:I{last}`），視為 Excel 內部註記、不上印出的紙本通知單；
        如使用者事後想讓紙本也印出，再擴大 print_area
- [ ]（Phase B）`record.py` 明細表加一欄保存兼課／第八節資訊：
      在 `DETAIL_HEADERS` 加一欄「鐘點類別」（值：`兼課` / `第八節` / 空白，代課列才有值，
      調課列固定空白），`_rebuild_stats` 的 `月統計` 再加兩欄
      `COUNTIFS` 公式：「代課(兼課)堂數」「代課(第八節)堂數」，方便對帳（此為提案，
      實作時再與使用者核對欄名／要不要拆更細）
