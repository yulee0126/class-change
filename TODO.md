...
## P7 — 協同教學（教師配當表）　✅ 完成（2026-09-05 提出並完成，F1→F2→F3→F4→F5 全部做完）
...
### F2 協作調課按鈕　✅ 完成（**格式後來發現有誤，見下方 F5 修正**）
...
### F3 協同獨立授課（代課情境）　✅ 完成
- [x] 情境：協同課其中一位老師請假，另一位協同老師直接單獨上課，不找外部代課
- [x] **仍然產生一張通知單**，但備註措辭改成「獨立授課」（不寫「XX老師代課」）：
      原老師列＝「改由{協同老師}老師獨立授課（原協同）」、協同老師列＝「獨立授課（原與
      {原老師}老師協同）」、班級列＝「{協同老師}老師獨立授課（原協同）」（措辭若不合適
      之後再調整，純文字改動不影響資料）
- [x] `SubLeg` 新增 `is_co_teach: bool`（同 `from_swap` 的做法，含序列化 round-trip），
      `builder.py` 據此改寫三處備註文字
- [x] 「＋新增代課」表單：填了原老師＋日期＋節次，若剛好比對出協同節次
      （`ctl.co_teachers_of()`），自動帶出建議代課老師＝協同老師、自動勾選
      「協同獨立授課」核取方塊（使用者仍可手動取消／改代課老師）
...
### F5 協作調課改成真正的資料模型（修正 F2 的格式錯誤）　✅ 完成
- 背景：使用者拿真實案例 `1002-趙瑋1150903.xlsx` 測試 F2 做出來的協作調課，
  發現輸出格式不對——F2 把協作調課拆成兩筆獨立 `SwapLeg`（甲↔目標一筆、
  乙↔目標一筆），導致**目標老師（張宥恩）被拆成兩張幾乎一樣的通知單／班級單
  重複兩行**，而真實案例是「一堂課只要一行」。使用者進一步指出：(1) 通知單
  要先印「要求調課」的一方（趙瑋、周蓁妍），再印「被調課」的一方（張宥恩）；
  (2) 要考慮反過來的情況——如果某老師是拿「另一堂協作課」去跟別人調，
  這時候反而是被調課那側才是協同班
- [x] 新資料型別 `CoSwapLeg`（`models.py`）：**完全對稱設計**，不分「哪一側才是
      協同班」——`teachers_a: list[str]` / `teachers_b: list[str]`，A、B 兩側都可以
      放 1 位或多位老師，同一份資料涵蓋一般情況、任一側協同、甚至兩側都協同
      的情況；`Leg = Union[SwapLeg, SubLeg, CoSwapLeg]`，序列化 round-trip
      （`leg_to_dict`/`leg_from_dict`）新增 `"coswap"` 分支
- [x] `builder.py`：一個 `CoSwapLeg` 只產生「A 側每位老師一列＋B 側每位老師
      一列」（不是 N×M 重複展開）；**先處理 teachers_a（要求調課方）再處理
      teachers_b（被調課方）**，符合印製順序要求；同側多位老師的備註會合併列出
      對方姓名（例如「與趙瑋/周蓁妍老師 基礎雜糧加工實作 調課」）；`validate()`
      加上 CoSwapLeg 分支（含跨側同名老師檢查、時段檢查）
- [x] `record.py`：`event_to_rows()` 加 CoSwapLeg 分支，A 側每人一列、B 側每人
      一列，原教師欄視情況合併多個姓名
- [x] `xlsx_reader.py`：`_pair_entries` 改成**分組演算法**（不是舊的貪婪配對）——
      同一 `(班級, {新時段, 原時段})` 的列先分成方向 A／B 兩組，兩組人數都
      ≥1 且**同一組內每個人兩兩互為確認協同老師**才判定為 `CoSwapLeg`；否則
      整組列為配不出（寧可老實報錯，不要贏面猜錯——真實資料驗證發現，舊的
      貪婪演算法會把「趙瑋、周蓁妍都聲稱要跟林冠佑調課」這種無法從協同資料
      消歧的情況，靜靜地隨機配給第一個遇到的老師，新演算法會正確地把 3 列
      都標成配不出，不會給錯誤的合併結果）
- [x] GUI `_CoSwapForm` 全面重寫成 A／B 兩側完全對稱的介面：每側都是「主要
      老師＋該側其他老師（自動偵測協同老師、可編輯／清空）＋科目＋日期＋節次」，
      同一份表單處理一般情況跟任一側協同的情況；支援編輯既有的 `CoSwapLeg`
      （`leg_form_data`/`update_leg` 都已支援 `"coswap"` kind）
- [x] `controller.leg_summaries()` 補上 CoSwapLeg 分支（原本會在協作調課筆數
      顯示於清單時噴 `AttributeError`，因為舊分支假設所有 leg 都有
      `.orig_teacher`/`.sub_teacher` 等 SwapLeg／SubLeg 專屬欄位）
- [x] 測試涵蓋 builder／record／xlsx_reader（含真實檔案）／controller／GUI
      端到端，全部改用新的對稱簽章 `add_co_swap(klass=, teachers_a=, subject_a=,
      date_a=, period_a=, teachers_b=, subject_b=, date_b=, period_b=)`
