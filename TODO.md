...
### F2 協作調課按鈕　⬜ 待做
- [ ] 使用者決定：**做成獨立按鈕**（不是塞進現有「新增調課」表單裡自動偵測），例如
      「＋協作調課」，跟「先調再代」同一層級
- [ ] 表單欄位（草案，待實作時再細看）：班級、科目、原時段（協同的兩人共用同一份）、
      協同的甲乙老師（可由 F1 的 `co_teachers` 資料自動帶出建議）、目標老師、目標科目、
      目標時段——一次送出展開成兩筆獨立 `SwapLeg`（甲↔目標一筆、乙↔目標一筆）
- [ ] 使用者已確認理解並接受：印出來會是「兩位協同老師各自一張通知單、內容很接近」，
      跟他過去手動做的 `1002-趙瑋1150903.xlsx` 一樣，不強求合併成一列
